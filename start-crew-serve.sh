#!/usr/bin/env bash
# start-crew-serve.sh — bring up containerized serve processes for all of cookbook-demo's
# crew roles (safety, structure, presentation, review), each wired to its real
# handlers/*.sh via CADS-Tunnel's scripts/channel-ops/serve-role-container.sh (#219, #117).
#
# Per-role keys/grants come from ENV_FILE (SERVE_COOKBOOK_<ROLE>_HOLDER_KEY/_NOISE_KEY/_GRANT),
...
IMAGE="${IMAGE:-cads-cookbook-demo-cookbook-agent:latest}"
DOCKER_NETWORK="${DOCKER_NETWORK:-cookbook-demo_default}"

ROLES="safety:safety_check:safety-check-handler.sh structure:text_generation:ingredients-handler.sh presentation:text_generation:presentation-handler.sh review:safety_check:review-handler.sh"
...
if [ "${1:-}" = "--selftest" ]; then
  ...
  for entry in $ROLES; do
    ...
    CONTAINER_NAME="cookbook-${role}-serve" \
      "$CORE_SCRIPT" --selftest || ok=0
  done
  [ "$ok" = "1" ] || die "one or more roles failed selftest"
  echo "start-crew-serve: all cookbook-demo crew roles passed selftest"
  exit 0
fi

: "${ENV_FILE:?set ENV_FILE (path to an env file with SERVE_COOKBOOK_<ROLE>_HOLDER_KEY/_NOISE_KEY/_GRANT per role)}"
...
for entry in $ROLES; do
  ...
  holder_var="SERVE_COOKBOOK_${role_upper}_HOLDER_KEY"
  noise_var="SERVE_COOKBOOK_${role_upper}_NOISE_KEY"
  grant_var="SERVE_COOKBOOK_${role_upper}_GRANT"
...
echo "start-crew-serve: all cookbook-demo crew roles started (docker ps | grep cookbook- to check status)"
